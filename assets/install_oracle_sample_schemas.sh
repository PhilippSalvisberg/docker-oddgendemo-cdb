...
gosu oracle bash -c "mkdir /tmp/log && cd ${ORACLE_HOME}/demo/schema && echo "EXIT" | sqlplus -s -l system/${PASS}@${PDB_NAME} @mksample ${PASS} ${PASS} hr oe pm ix sh bi users temp /tmp/log/ ${PDB_NAME}"
# reset ORACLE_HOME
